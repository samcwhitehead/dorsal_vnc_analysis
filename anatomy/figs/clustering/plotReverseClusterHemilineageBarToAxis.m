% -------------------------------------------------------------------------
% function to make bar plots showing hemilineages across cell
% collection broken up by IN cluster numbers (reverse of other plot type,
% which shows hemilineage makeup of clusters)
%
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
...
% plot form
% -------------------------------------------------------------------------
function ax = plotReverseClusterHemilineageBarToAxis(ax, conn_clust_struct, ...
   useSubTypeFlag,colorMode, sortStackFlag, fullLabelFlag, horizontalFlag)
% ---------------------
%% inputs and params
...
if ~exist('conn_clust_struct','var') || isempty(conn_clust_struct)
    ...
    clustDir = dir(fullfile(rootPath, 'conn_clust_struct*.mat')) ; 
    clustFn = clustDir(1).name ;
...
hemilineages_all = {conn_clust_struct.hemilineageID} ;
neurons_all = {conn_clust_struct.neuron_label} ;
IDs_all = {conn_clust_struct.ID} ;
% --------------------------------------------------------
%% parse hemilineages
...
for k = 1:length(hemilineages_all)
    ...
    elseif contains(hemilineages_all{k}, 'emb','IgnoreCase',true)
        ...
        tokenNames = regexp(hemilineages_all{k}, searchExp,'names') ;
...
if useSubTypeFlag
    % format for hemilineage ID
    hl_for_plot = arrayfun(@(x) strcat(x.NB, x.notchType, x.subType), ...
        parse_struct, 'UniformOutput', 0) ;
    % string we'll use to get NB number in regexp
    NB_num_str = ['(?<NB>\d*)(?<notchType>[AB])' ...
        '(?<subType>[' greek_str ']?)'] ; 
...
% now get the unique hemilineages across cells
hl_unique = unique(hl_for_plot,'stable') ;
N_hl = length(hl_unique) ; 

% want the sorted version of these as well -- annoyingly, have to add
% digits to some to get sorting to work
hl_unique_2digit = hl_unique ; 

% feeling lazy so just loop
for q = 1:length(hl_unique_2digit)
    % current hemilineage
   hl_curr = hl_unique_2digit{q} ; 
   
   % ignore abdominal and embryonic
   if ~(contains(hl_curr, 'abd','IgnoreCase',true) || ...
           contains(hl_curr, 'emb','IgnoreCase',true))
       % if we have a neuroblast number, read it out, get 
       tokenNames = regexp(hl_curr, NB_num_str ,'names') ;
       NB_num_new = num2str(str2double(tokenNames.NB),'%02d') ; 
       tokenNames.NB = NB_num_new ; 
       
       % reconstruct hemilineage string
       if useSubTypeFlag
           hl_unique_2digit{q} = strcat(tokenNames.NB, ...
               tokenNames.notchType, ...
               tokenNames.subType) ;
       else
           hl_unique_2digit{q} = strcat(tokenNames.NB, ...
               tokenNames.notchType) ;
...
% sort 2-digit neuroblast number hemilineage strings -- we'll use this
% order for plots
[hl_unique_sort, sort_ind] = sort(hl_unique_2digit) ; 

% ---------------------------------------------------------
%% loop over hemilineages and get cluster make-up
% this will be in the form of a matrix with rows corresponding to clusters
% and columns corresponding to hemlineages
bar_count_mat = zeros(N_clust, N_hl) ; 
for k = 1:length(hl_unique)  % loop over hemilineages
